...
public class UserController extends BaseSecurityController{
    ...
    public Result<Object> regist(@ModelAttribute User u,
        ...
        if(StrUtil.isBlank(u.getUsername()) || StrUtil.isBlank(u.getPassword()) || StrUtil.isBlank(u.getDepartmentId())){
            return new ResultUtil<Object>().setErrorMsg("缺少必需表单字段");
        }
    ...
    @ApiOperation(value = "通过部门ID查询人员")
    public Result<Object> getByDepartmentId(@ModelAttribute User user){

        List<User> userList = userService.findByDepartmentId(user.getDepartmentId());
